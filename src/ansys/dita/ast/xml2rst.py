...
from ansys.dita.ast import folder_variables as var
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--command", help="Convert a single command") # not accepted
    ...
    graph_path, link_path, term_path, book_path = path.get_paths(xml_path)

    links = load.load_links(link_path)
    fcache = load.load_fcache(graph_path)
    docu_global = load.load_docu_global(term_path)
    terms = load.load_terms

    variables = var.AutogeneratedFolder(terms)

    # links, base_url = chap2.write_chapt(doc_path)
